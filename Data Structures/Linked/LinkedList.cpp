...
class SLLNode
{
	public:
			int data;
			SLLNode* next;
			SLLNode(int _data=0,SLLNode* _next=NULL)
			{ 
			 	data=_data;
			 	next=_next;
...
int SLL :: deleteBefore(int ele)
{
	...
	{
	    throw "   Linked list has only one element";
		 
	}
	else if(temp->data==ele)
	{
		throw "  There is no element before Head(First Element) ";
...
{	
	...
	{
    	throw "  There is no Element After Tail(Last Element) ";
